...
<div class="main">
  ...
  <div class="page" id="page-grammar">
    <div>
      ...
      <div class="grammar-subtitle">Select a tone, paste your text, and transform it — fully offline.</div>
    </div>
...
function showGrammarResult(correctedText, changeCount) {
  var btn = document.getElementById("grammar-fix-btn");
...
class Dashboard:
    ...
    def _handle_grammar(self, text, mode="fix"):
        ...
        self._js(f"showGrammarResult({json.dumps(corrected)}, {changes})")
    ...
    def _improve(self, text):
        ...
        for pat, rep in spoken:
            text = re.sub(pat, rep, text, flags=re.IGNORECASE)
        for filler in [r"\bbasically\b", r"\bactually\b", r"\bliterally\b", r"\blike\b"]:
        ...
        redundant = [
            ...
            (r"\bsafe and secure\b",        "secure"),
        ]
        for pat, rep in redundant:
            text = re.sub(pat, rep, text, flags=re.IGNORECASE)
        weak = [
            (r"\bis able to\b",             "can"),
            (r"\bare able to\b",            "can"),
